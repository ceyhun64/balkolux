...
const BlogCardSkeleton = memo(() => (
  <div className="space-y-8">
    <div className="aspect-[16/10] w-full bg-stone-50 animate-pulse" />
    <div className="space-y-4">
      <div className="h-2 w-20 bg-stone-100 animate-pulse" />
      <div className="h-8 w-full bg-stone-100 animate-pulse" />
      ...
      <main className="max-w-6xl mx-auto px-6 py-24 md:py-32">
        ...
        <header className="mb-24 text-center space-y-4">
          <motion.span
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="text-[10px] tracking-[0.4em] uppercase text-stone-400 font-medium"
          >
            BalkoLüx Journal
          </motion.span>
          <motion.h1
          ...
          <div className="w-10 h-[1px] bg-stone-200 mx-auto mt-8" />
        </header>
            ...
            : blogs.map((blog, index) => (
                ...
                >
                  <Link href={`/blog/${blog.id}`} className="space-y-6">
                    ...
                    <div className="relative aspect-[16/11] overflow-hidden bg-stone-50">
                      <img
                        src={blog.image}
                        alt={blog.title}
                    ...
                    <div className="space-y-3">
                      <span className="text-[9px] tracking-[0.2em] uppercase text-stone-400 font-semibold">
                        {blog.category}
                      </span>
                      <h2 className="text-xl font-serif font-light leading-snug group-hover:text-stone-600 transition-colors">
                        {blog.title}
                      </h2>
        ...
        <section className="mt-40 border-t border-stone-100 pt-24 text-center max-w-xl mx-auto">
          ...
          <form className="flex gap-4 border-b border-stone-200 pb-2 transition-focus-within focus-within:border-stone-900">
            <input
              type="email"
              placeholder="E-posta adresiniz"
              className="bg-transparent border-none w-full text-sm outline-none placeholder:text-stone-300 font-light"
            />
            <button className="text-[10px] uppercase tracking-widest font-bold whitespace-nowrap hover:text-stone-500 transition-colors">
              Kayıt Ol
            </button>
          </form>
        </section>
