...
export default function TableSets() {
  ...
  useEffect(() => {
    async function fetchProducts() {
      try {
        const res = await fetch("/api/products/category/2");
  ...
  return (
    <div className="min-h-screen bg-zinc-100 text-stone-900 selection:bg-stone-900 selection:text-white overflow-x-hidden relative">
      <style jsx global>{`
      ...
      <main className="container mx-auto px-6 md:px-12 py-24 lg:py-24 relative z-10">
        ...
        <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-8 border-b border-stone-200 pb-8">
          {/* Desktop Filter Toggle */}
          <button
            onClick={() => setIsFilterOpen(!isFilterOpen)}
            className="hidden lg:flex group items-center gap-3 modern-sans text-[11px] tracking-[0.2em] uppercase text-stone-500 hover:text-black transition-all"
          >
            <div
              className={cn(
                "p-2 border border-stone-200 rounded-full transition-colors group-hover:bg-stone-900 group-hover:text-white",
                isFilterOpen && "bg-stone-900 text-white"
              )}
            >
          ...
          <button
            ...
            className="lg:hidden group flex items-center gap-3 modern-sans text-[11px] tracking-[0.2em] uppercase text-stone-500 hover:text-black transition-all"
          >
            <div className="p-2 border border-stone-200 rounded-full transition-colors group-hover:bg-stone-900 group-hover:text-white">
              <SlidersHorizontal size={14} />
            </div>
            Filtreler
          </button>
        ...
        <div className="flex flex-row items-start">
          ...
          >
            {filteredProducts.length > 0 ? (
              ...
              <div className="h-[40vh] flex flex-col items-center justify-center border border-dashed border-stone-200">
                <p className="luxury-serif text-2xl italic text-stone-400">
                  Sonuç bulunamadı.
                </p>
              </div>
            )}
      ...
      <AnimatePresence>
        {isMobileFilterOpen && (
          <>
            ...
            >
              {/* Sheet Header */}
              <div className="flex items-center justify-between px-6 py-5 border-b border-stone-200 sticky top-0 bg-white z-10">
                <div className="flex items-center gap-3">
              ...
              <div className="sticky bottom-0 bg-white border-t border-stone-200 px-6 py-4">
                <button
                  onClick={() => setIsMobileFilterOpen(false)}
                  className="w-full bg-stone-900 text-white py-4 rounded-xl modern-sans text-[12px] font-medium tracking-wider uppercase hover:bg-stone-800 transition-colors"
                >
                  Filtreleri Uygula
                </button>
              </div>
