          ...
          {cartItems.length === 0 ? (
            ...
            >
              <div className="mb-8 relative">
                ...
                />
                <div className="absolute -top-2 -right-2 h-4 w-4 bg-zinc-100 rounded-full animate-pulse" />
              </div>
              <h1 className="text-2xl font-light tracking-tight text-zinc-900 mb-3">
                Sepetiniz Boş
              </h1>
              <p className="text-zinc-400 text-sm max-w-[280px] mb-10 leading-relaxed font-light">
                Koleksiyonlarımıza göz atarak size en uygun parçaları keşfedin.
              </p>
              <Link href="/products">
                <Button className="bg-zinc-950 hover:bg-zinc-800 text-white rounded-none px-10 py-6 text-xs tracking-widest transition-all duration-500">
                  KEŞFETMEYE BAŞLA
                </Button>
              </Link>
            </motion.div>
            ...
            <div className="max-w-7xl mx-auto">
              <header className="flex flex-col items-start mb-12 border-b border-zinc-100 pb-8">
                <Link
                  href="/products"
                  className="group flex items-center gap-2 text-zinc-400 hover:text-zinc-900 transition-colors mb-6 text-xs tracking-widest"
                >
                  <ArrowLeft
                    size={14}
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-16">
                ...
                <div className="lg:col-span-4 relative">
                  <div className="sticky top-32">
                    ...
                    <p className="mt-6 text-[11px] text-zinc-400 leading-relaxed text-center font-light px-4">
...
function CartLoadingSkeleton() {
  return (
    <div className="container mx-auto px-6 py-24">
      <Skeleton className="h-12 w-48 mb-12 bg-zinc-50" />
      <div className="flex flex-col lg:flex-row gap-16">
        <div className="flex-1 space-y-8">
          {[1, 2, 3].map((i) => (
            <div key={i} className="flex gap-6 pb-8 border-b border-zinc-50">
              <Skeleton className="w-32 h-40 bg-zinc-50" />
              <div className="flex-1 space-y-4 py-2">
                <Skeleton className="h-4 w-1/3 bg-zinc-50" />
                <Skeleton className="h-3 w-1/4 bg-zinc-50" />
                <Skeleton className="h-8 w-24 mt-4 bg-zinc-50" />
              </div>
            </div>
          ))}
        </div>
        <Skeleton className="w-full lg:w-[380px] h-[400px] bg-zinc-50 rounded-none" />
      </div>
    </div>
  );
}
