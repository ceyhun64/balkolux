    ...
    <div className="group flex flex-row w-full gap-6 py-8 bg-white border-b border-zinc-100 last:border-0 transition-all">
      ...
      <div className="flex-1 flex flex-col justify-between py-1">
        <div className="flex justify-between items-start">
          <div className="space-y-1">
            <Link href={`/products/${product.id}`}>
              <h3 className="text-sm md:text-base font-light tracking-tight text-zinc-900 hover:text-zinc-500 transition-colors uppercase">
                {product.title}
              </h3>
            </Link>
          ...
          <button
            onClick={onRemove}
            className="text-zinc-300 hover:text-zinc-950 transition-colors p-1"
            aria-label="Kaldır"
          >
        ...
        <div className="flex flex-row justify-between items-end">
          ...
          <div className="flex items-center border border-zinc-200 px-2 py-1">
            <button
              onClick={onDecrease}
              disabled={quantity <= 1}
              className="p-1 text-zinc-400 hover:text-zinc-950 disabled:opacity-20 transition-colors"
            >
          ...
          <div className="text-right">
            <p className="text-[10px] text-zinc-400 font-light mb-1">
              Birim: ₺{(product.price || 0).toLocaleString("tr-TR")}
            </p>
            <span className="text-sm md:text-base font-medium tracking-tight text-zinc-950">
