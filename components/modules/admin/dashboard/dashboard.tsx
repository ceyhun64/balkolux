...
export default function AdminDashboard() {
  ...
  const getStatusBadge = (status: string) => {
    ...
    return (
      <Badge
        variant="outline"
        className={`${style} flex items-center w-fit gap-1.5 px-2.5 py-0.5 rounded-full text-[11px] font-medium border`}
      >
        {icon} {label}
      </Badge>
    );
  ...
  return (
    <div className="flex min-h-screen bg-[#F8F9FB] font-sans selection:bg-indigo-100">
      ...
      >
            ...
            <Link key={card.id} href={card.href}>
              <Card className="bg-white border-none shadow-[0_2px_8px_rgba(0,0,0,0.04)] hover:shadow-[0_8px_20px_rgba(0,0,0,0.06)] transition-all duration-300 group rounded-2xl">
                <CardHeader className="flex flex-row items-center justify-between pb-2 space-y-0">
                  ...
                  <ArrowRight
                    size={14}
                    className="text-slate-300 opacity-0 group-hover:opacity-100 transition-all translate-x-[-10px] group-hover:translate-x-0"
                  />
                </CardHeader>
                <CardContent>
        ...
        {/* RECENT ORDERS TABLE */}
        <Card className="bg-white border-none shadow-[0_2px_8px_rgba(0,0,0,0.04)] rounded-2xl overflow-hidden">
              ...
              <table className="w-full text-left">
                <thead className="bg-slate-50/50 text-slate-400 text-[11px] uppercase tracking-wider font-semibold">
                  <tr>
                    ...
                    <th className="px-6 py-3 hidden sm:table-cell">Ürünler</th>
                      ...
                      <td className="px-6 py-4 text-right">
                        <Link href={`/admin/orders?orderId=${order.id}`}>
                          <Button
                            size="sm"
                            variant="ghost"
                            className="h-8 w-8 p-0 rounded-full hover:bg-slate-900 hover:text-white transition-all"
                          >
                            <ArrowRight size={14} />
                          </Button>
                        </Link>
